...
### Memory management in the self-hosted backend: partly there (2026-08-16)

`sh2` still leaks. What landed is the whole layout change plus a
complete reference-counting implementation that is **switched off**,
because it isn't sound yet and the fixed point matters more.

#### Landed and verified

**Every heap cell now carries a refcount word**, and every offset is
...
One allocation point (`@plum_alloc`) initialises it to 1. The fixed
point holds with the new layout, 16/17 exec_corpus, everything green.

That change alone found a bug worth recording: two of the offset edits
silently didn't apply, and **the exec_corpus did not notice** — 16/17
still passed. `bootstrap-check` caught it immediately, because the
compiler's own source exercises enum construction and matching far
harder than any fixture. The fixed point is not just a milestone; it is
the most sensitive test in the project.

#### Written, and off by default

The ownership discipline, stated once and implemented:
...
4. A call CONSUMES its arguments; a returned value carries its +1 out.
...
the real backend's FBIP pass would sit on top of exactly this.

Supporting it: per-type release functions generated by the same
worklist that generates equality functions and specializations (with the
same `seen` guard, so a recursive type gets one function that calls
itself); null-initialised slots so release-on-overwrite is safe on the
first write, which is what makes a `let` inside a loop free the previous
iteration's value.

**Where the typed tree pays off again.** `plum-ir/src/fbip.rs` cannot
track function parameters at all — its own comment gives the reason,
"no type checker in this IR to prove one is heap-shaped", and that is
the documented root cause of the gap-1 work reverted earlier in this
project. Here every node's type is known, so `cg_is_heap` is total and
exact, and parameters are counted like anything else.

#### Why it is off

With counting on, 16/17 exec_corpus fixtures pass — but **the stage-2
compiler segfaults while compiling the compiler.** At least one release
is still unsound on code the corpus does not reach. Candidates, in
order of suspicion: `Array.push`/`set` copy element POINTERS with
`memcpy` without incrementing them, so two arrays share elements under
one count; closure captures are stored without incrementing, and a
closure's release cannot walk them (its captures aren't visible from its
type — the reason the real backend stores a release-function pointer in
every closure cell); and arguments to runtime primitives are never
released, which is a leak rather than a crash but muddies the accounting.

`cg_rc_enabled` gates every inc, dec, null-init and slot release. With
it `false` the emitted IR is exactly the leaking version whose fixed
point is verified. Flip it to `true` to work on the bug;
`./bootstrap/bootstrap-check` is the test that has to pass before it
can be flipped for good.

Shipping correct-and-leaking over fast-and-broken is the whole call
here, and the switch is what makes it a decision rather than a
half-finished edit.
