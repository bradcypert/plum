...
## Enums that hold references (2026-08-26)

The last of the four gaps. An enum whose payload is a `String` went from
1001 allocations to 1 — one, not two, because the same string literal is
stored back each iteration and never reallocated either.

### The companion function, and why enums get one when structs do not

A struct's children are dropped inline at the reuse site: its layout is
fixed, so that is three instructions per heap field, emitted only where
a reuse actually happens. An enum's children depend on its runtime
**tag**, so the same walk is a whole `switch` — inlining that at every
reuse site is how a small optimisation turns into a large module.

So enums delegate to `@plum_relc_<type>`, generated by
`cg_emit_relc_fn`: the tag switch `@plum_rel_<type>` already contains,
with the dec-and-free wrapper removed. It reuses `cg_rel_cases` and
`cg_rel_blocks` unchanged, which is the point — there is one definition
of "which payloads does this variant own" and both functions are built
from it.

Generated only for enums that actually own something, which is what
keeps this from becoming a parallel family of 301 mostly-dead
functions. The seed grew 1.3%.

### Changing variant is the case worth a fixture

Recycling a cell across variants means the new tag may use fewer
payload slots than the old one. Those slots keep dangling pointers to
things the drop just released. That is safe for one reason only: a
payload is loaded only under a matching tag, and the tag now says
otherwise. `reuse_enum_payload` cycles `Blank -> Text -> Count -> Blank`
so the fixture actually crosses those transitions rather than asserting
they are fine.

The other half is the same inc-before-dec property the struct case
relies on, and it holds for the same reason: `cg_pat_payloads` binds a
payload with an increment, so `Text(s) => Text(s)` has the string at two
references before the old cell's is dropped.

### Where the memory model stands

| shape | allocations per 1000 rebuilds |
|---|---|
| struct, scalar fields | 2 |
| struct, `String` field | 2 |
| enum, scalar payload | 2 |
| enum, `String` payload | 1 |
| `Array.map` chain | 1 array cell |
| `Array.push` accumulation | 10 (amortised doubling) |
| `String.concat` accumulation | 65 (amortised doubling) |

What remains is `Array.filter` (writes a shorter result, so its length
header needs patching on the reuse path), mapping an array whose
elements are references (a different loop *body* — element `i` has to be
released after the closure has taken its own reference), and the closure
cell allocated per iteration for a literal inside a loop. That last one
is not a reuse problem at all: a closure that captures nothing is the
